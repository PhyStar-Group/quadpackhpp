...
namespace quadpack {
    ...
    T_real Quadpack<T_fun, T_real>::qagp(T_fun f, T_real user_data[], T_real a, T_real b, int npts2, T_real* points, T_real epsabs, T_real epsrel, T_real* abserr, int* neval, int* ier)
    {
        ...
        T_real defab2, dres, elist[LIMIT], erlarg, erlast, errbnd;
        T_real errmax, error1, error2, erro12, errsum, ertest, ndin[40];
        T_real pts[40], resa, resabs, reseps, result, res3la[3];
        T_real rlist[LIMIT], rlist2[52], sign, temp;
    ...
    _40:
        ...
        for (i = 0; i < nint; i++) {
            b1 = pts[i + 1];
            area1 = gk21(f, user_data, a1, b1, &error1, &defabs, &resa);
        ...
        *neval = 21 * nint;
        ...
        if ((*abserr <= 100.0 * epmach * resabs) && (*abserr > errbnd))
            *ier = 2;
        if (nint == 0)// fortran version is if nint.eq.1
            goto _80;
        for (i = 0; i < npts; i++) {
            jlow = i + 1;
            ind1 = iord[i];
    ...
    _80:
        ...
        *abserr = oflow;
        ...
        for (last = npts2-1; last <= limit; last++) { //CZW: last should be C style
            ...
            b2 = blist[maxerr];
            erlast = errmax;
            area1 = gk21(f, user_data, a1, b1, &error1, &resa, &defab1);
            area2 = gk21(f, user_data, a2, b2, &error2, &resa, &defab2);
        ...
        _150:
            ...
            if (*ier >= 5) goto _170; //CZW:fortrn version is ier.ge.5
        _155:
            maxerr = iord[0];
            errmax = elist[maxerr];
            nrmax = 0;
