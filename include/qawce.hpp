 ...
 *  PARAMETERS:
 *
 *      f() -   T_real precision function defining the integrand.
 *
...
namespace quadpack {
    ...
    T_real Quadpack<T_fun, T_param, T_real>::qawce(T_fun f, T_param user_data[], T_real a, T_real b, T_real c, T_real epsabs,
        T_real epsrel, T_real* abserr, int* neval, int* ier)
    {
        T_real aa, area, area1, area2, area12, a1, a2, bb, b1, b2;
        T_real errbnd, errmax, error1, error2, erro12, errsum, result;
    ...
    _10:
        *ier = 0;
        krule = 1;
        result = qc25c(f, user_data, aa, bb, c, abserr, &krule, neval);
        rlist[0] = result;
        elist[0] = *abserr;
        iord[0] = 0;
        alist[0] = a;
        ...
        for (last = 1; last < limit; last++) {
            ...
            a2 = b1;
            krule = 2;
            area1 = qc25c(f, user_data, a1, b1, c, &error1, &krule, &nev);
            *neval = *neval + nev;
            area2 = qc25c(f, user_data, a2, b2, c, &error2, &krule, &nev);
            *neval = *neval + nev;
