...
/* QAGI - Integration over (semi-) infinite intervals. (From QUADPACK)
 *
...
namespace quadpack {
    ...
    T_real Quadpack<T_fun, T_real>::qagi(T_fun f, T_real user_data[], T_real bound, int inf, T_real epsabs, T_real epsrel, T_real* abserr, int* neval, int* ier)
    {
        ...
        T_real erlast, errbnd, errmax, error1, error2, erro12;
        T_real errsum, ertest, resabs, reseps, result, res3la[3];
        T_real alist[LIMIT], blist[LIMIT], elist[LIMIT], rlist[LIMIT];
        T_real rlist2[52], small = 0; /* small will be initialized in _80 */
        ...
        *abserr = 0.0;
        ...
        result = gk15i(f, user_data, boun, inf, 0.0, 1.0, abserr, &defabs, &resabs);
        ...
        *abserr = oflow;
        ...
        for (last = 1; last <= limit; last++) {
            ...
            b2 = blist[maxerr];
            erlast = errmax;
            area1 = gk15i(f, user_data, boun, inf, a1, b1, &error1, &resabs, &defab1);
            area2 = gk15i(f, user_data, boun, inf, a2, b2, &error2, &resabs, &defab2);
