...
namespace quadpack {
        ...
        T_real* abserr, int* neval, int* ier, int* momcom, T_real** chebmo)
    {
        ...
        T_real a1, a2, b1, b2, correc, defabs, defab1;
        T_real defab2, domega, dres, erlarg, erlast, errbnd;
        T_real errmax, error1, error2, erro12, errsum, ertest;
        T_real resabs, reseps, result, res3la[3];
        T_real alist[LIMIT], blist[LIMIT], rlist[LIMIT];
        T_real elist[LIMIT], rlist2[52], small, width;
    ...
    _10:
        ...
        for (last = 1; last < limit; last++) {
            ...
            b2 = blist[maxerr];
            erlast = errmax;
            area1 = qc25o(f, user_data, a1, b1, domega, sincos, nrmom, maxp1, 0,
                &error1, &nev, &resabs, &defab1, momcom, chebmo);
            *neval += nev;
            area2 = qc25o(f, user_data, a2, b2, domega, sincos, nrmom, maxp1, 1,
