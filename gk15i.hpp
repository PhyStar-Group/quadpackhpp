...
namespace quadpack {
    ...
    T_real Quadpack<T_fun, T_param, T_real>::gk15i(T_fun f, T_param user_data[], T_real boun, int inf, T_real a, T_real b, T_real* abserr, T_real* resabs, T_real* resasc)
    {
       ...
       0.00000000000000000000 };
        ...
        T_real fv1[8], fv2[8];
        T_real absc, absc1, absc2, centr, dinf;
        T_real fc, fsum, fval1, fval2, hlgth, resg, resk;
        T_real reskh, result, tabsc1, tabsc2;
        int j;

        dinf = min((T_real)(1.0), (T_real)inf);
        centr = 0.5 * (a + b);
        hlgth = 0.5 * (b - a);
